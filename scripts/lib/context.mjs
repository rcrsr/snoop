...
/** 1_000_000 -> "1M", 114_238 -> "114k", 940 -> "940". */
export function abbreviateTokens(n) {
  if (n >= 1_000_000) {
    const m = n / 1_000_000
...
/**
 * `ctx 11% (114k/1M) o5` — occupancy, window, and the model the reading is
 * measured against, since the same tokens are 11% of opus[1m] and 57% of a 200k
 * model. An assumed window is prefixed `~`: the token count is exact but the
 * denominator is a guess, and a confident percentage over a guessed window is
 * worse than an obviously hedged one. Peak is appended only when it differs
 * from current, which happens when the session compacted.
 */
export function formatContextSummary(ctx, shortcode = (m) => m) {
  if (!ctx) return ''

  const tilde = ctx.windowBasis === 'assumed' ? '~' : ''
  const model = ctx.model ? ` ${shortcode(ctx.model)}` : ''
  const parts = [
    `ctx ${tilde}${ctx.usedPercentage}% (${abbreviateTokens(ctx.used)}/${abbreviateTokens(ctx.size)})${model}`,
  ]

  if (ctx.peak > ctx.used) parts.push(`peak ${tilde}${ctx.peakPercentage}%`)
  for (const c of ctx.compactions ?? []) {
    parts.push(`${c.trigger} compact −${abbreviateTokens(c.droppedTokens)}`)
  }
...
/**
 * `sub ctx: Explore s5 366k / claude o48 188k` — one reading per subagent,
 * largest first. The agent type is what makes a reading traceable back to the
 * work that caused it, so it leads when snoop resolved a name for the id.
 */
export function formatSubagentContext(subagents, shortcode = (m) => m, limit = 4) {
  if (!subagents.length) return ''

  const shown = subagents.slice(0, limit).map((s) => {
    const models = s.models.map(shortcode).join('+') || '?'
    const name = s.name ? `${s.name} ` : ''
    return `${name}${models} ${abbreviateTokens(s.peak)}`
  })
  if (subagents.length > limit) shown.push(`+${subagents.length - limit} more`)

  return `sub ctx: ${shown.join(' / ')}`
}
